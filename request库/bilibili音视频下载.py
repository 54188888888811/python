...
def bilibili(uil, cookies):
    ...
    if input('获取音频 or 视频（1 or 2）') == '1':
        ...
        command = [
            'ffmpeg',
            '-y',  # 覆盖输出文件
            '-i', 'C:\pythonProject\\temp_video.m4s',  # 输入视频文件
            '-i', 'C:\pythonProject\\temp_audio.m4s',  # 输入音频文件
            '-c:v', 'copy',  # 视频复制，不重新编码
            '-c:a', 'aac',  # 音频编码为 AAC
        ...
        try:
            ...
            print(f"合并失败：{e}")
        try:
            os.remove('C:\pythonProject\\temp_video.m4s')
            os.remove('C:\pythonProject\\temp_audio.m4s')
